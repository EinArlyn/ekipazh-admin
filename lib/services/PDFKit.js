...
function formContent (order, factory, userDiscounts, additionalProductsIds, callback) {
   ...
   */
  function __formProductContent (orderAddElements, product, structCounter, laminations, addElements, glasses, productPrice, addElemPrice) {
    var content = '';
    var divider = '';
    ...
    for (var k = 0, len3 = orderAddElements.length; k < len3; k++) {
      if (orderAddElements[k].product_id === product.product_id) {
        if (mouldedSet.indexOf(addElements[orderAddElements[k].element_id].group_id) !== -1) {
          filteredAddElements.push((addElements[orderAddElements[k].element_id].name + ' (' + orderAddElements[k].element_width + ',' + orderAddElements[k].element_qty + 'шт.)'));
        } else if (pieceSet.indexOf(addElements[orderAddElements[k].element_id].group_id) !== -1) {
          filteredAddElements.push((addElements[orderAddElements[k].element_id].name + ' (' + orderAddElements[k].element_qty + 'шт.)'));
        } else {
          filteredAddElements.push((addElements[orderAddElements[k].element_id].name + ' (' + orderAddElements[k].element_width + 'х' + orderAddElements[k].element_height + ',' + orderAddElements[k].element_qty + 'шт.)'));
        }
      }
    }
