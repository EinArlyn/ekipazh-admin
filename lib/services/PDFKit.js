 ...
 * @callback done
 */
function parseOrder (order, factory, done, locale) {
  var loc = locale || i18n.getLocale();
  var t = function (phrase) { return i18n['__']({ phrase: phrase, locale: loc }); };
...
function formContent (order, factory, userDiscounts, currUserId, additionalProductsIds, callback, locale) {
  var loc = locale || i18n.getLocale();
  var t = function (phrase) { return i18n['__']({ phrase: phrase, locale: loc }); };
  var data = {
    structCounter: 1,
  ...
  models.users_discounts.find({
    ...
    parseOrder(order.order, factory, function (parsedOrder) {
      ...
      models.locales_names.findAll({
        ...
        async.eachSeries(products, function (product, cb) {
          ...
          if (product.is_addelem_only !== 2 && product.is_addelem_only !== 5) {
          ...
          const templateSource = JSON.parse(product.template_source);

          currencyValue = order.order.currency_value;
          
          let extraPrice = templateSource.extra_price ? templateSource.extra_price/currencyValue : 0;
          ...
          data.weightTotal += (templateSource.weightAddElements || 0) * product.product_qty + (templateSource.weightConstruction || 0) * product.product_qty;
          data.square += +(product.template_square * product.product_qty);
          data.basePrice += +(parseFloat((product.template_price + extraPrice*100 / (100 - order.discount_construct)) * product.product_qty).toFixed(2));
          data.basePriceDisc += +(parseFloat((product.template_price + extraPrice) * product.product_qty).toFixed(2) - parseFloat(parseFloat(product.template_price * (order.discount_construct / 100)) * product.product_qty)).toFixed(2);
          ...
          data.content += __formProductContent(
            order.order.order_addelements,
            product, 
            data.structCounter++, 
            parsedOrder.laminations, 
            parsedOrder.addElements, 
            parsedOrder.glasses, 
            parseFloat(parseFloat((+product.template_price+extraPrice) * product.product_qty) - ((parseFloat(product.template_price * parseFloat(order.discount_construct / 100))) * product.product_qty)), 
            (parseFloat(product.addelem_price * product.product_qty).toFixed(2)) - parseFloat(+parseFloat(product.addelem_price * (order.discount_addelem / 100)) * product.product_qty).toFixed(2),
            values, 
            prodGlassInfo, 
            dataObj, 
            heat_coef_norm,
            products_length,
            loc,
            order.user.country_nds);
          data.extraPriceGlobal = templateSource.extra_price_global ? templateSource.extra_price_global : 0;
          cb();
        }, function (err) {
          formAdditionals();
        });
    ...
    function formAdditionals () {
      ...
      async.eachSeries(additionals, function (additional, _cb) {
        ...
        currencyValue = order.order.currency_value;
        if(+currUserId !== +order.user.dataValues.id) {
          additional.element_price = additional.element_price/currencyValue * (1 + createUserDiss.default_add_elem / 100); ;
        } else {
          additional.element_price = additional.element_price/currencyValue
        }
        ...
        additionalContent += additional.name + '(' + length + height + parseInt(additional.element_qty, 10) + ' ' + t('pcs') + ')';
        ...
        filteredAddElements.push((addElementsCount + '. ' + translateAddElem + '<br>' + (!+additional.element_width ? '' : (t('Width') + ' ' + additional.element_width + ' ' + t('mm') + ' ' + ' / ')) + (!+additional.element_height ? '' : (t('Length') + ' ' + additional.element_height + ' ' + t('mm') + ' ' + ' / ')) + additional.element_qty + ' ' +t('pcs')));
         ...
         * 
              '<div>' + t('Amount') + ': ' +  + ' ' + '</div>' +
              '<div>' + t('width') + ': ' + parseInt(additional.element_width, 10) + ' ' + t('mm') + ' &nbsp;&nbsp; ' + t('height') + ': ' + parseInt(additional.element_height, 10) + ' ' + t('mm') + '</div>' +
              t('Perimeter') + ': ' + (((2 * (parseInt(additional.element_width, 10) + parseInt(additional.element_height, 10)))/1000) * additional.element_qty) + ' ' + t('m') +
              */
             _cb();
            }, function (err) {
              data.content += __formAdditionalContent(filteredAddElements, data.structCounter++, data.additionalPriceDisc, loc);
              callback(data);
            });
            });
          }
   ...
   */ 
  function __formProductContent (orderAddElements, product, structCounter, laminations, addElements, glasses, productPrice, addElemPrice, translateLists, prodGlassInfo, dataObj, normalCoef, products_length, locale, nds) {
    var loc = locale || i18n.getLocale();
    var t = function (phrase) { return i18n['__']({ phrase: phrase, locale: loc }); };
    var currentLanguage = loc;
    ...
    var weightConstruction = ((productTemplate.weightAddElements || 0)*product.product_qty + (productTemplate.weightConstruction || 0)*product.product_qty);
    var connectorsWidth = 0;
    var connectorsHeight = 0;
    var connectorsTotalWidth = 0;
    var connectorsTotalHeight = 0;
    ...
    if (product.is_addelem_only === 2 || product.is_addelem_only === 5) {
      ...
      productTemplate.details.forEach((block, index) => {
        ...
        if (block.newHandle && translateLists["lists"][block.newHandle]) {
        ...
        if (block.connectorsPoints && block.connectorsPoints.length) {
          block.connectorsPoints.forEach(point => {
            if (point.side === 'left' || point.side === 'right') {
              connectorsWidth += point.width;
            }
            if (point.side === 'top' || point.side === 'bottom') {
              connectorsHeight += point.width;
            }
          });
        }
      })
      connectorsTotalWidth = parseInt(product.template_width, 10) + connectorsWidth;
      connectorsTotalHeight = parseInt(product.template_height, 10) + connectorsHeight;
    }
    ...
    for (var j = 0, len2 = laminations.length; j < len2; j++) {
      ...
      if(laminations[j].id !== 1) {
        translateLaminName = translateLists["lamination_factory_colors"][laminations[j].id] && translateLists["lamination_factory_colors"][laminations[j].id][currentLanguage] || laminations[j].name;
      }
      ...
      if (product.lamination_in_id === laminations[j].lamination_type_id) {
        filteredLaminations.push((translateLaminName + ' (' + t('inner') + ')'));
      }

      if (product.lamination_out_id === laminations[j].lamination_type_id) {
        filteredLaminations.push((translateLaminName + ' (' + t('outer') + ')'));
      }
    }
    ...
    for (var k = 0, len3 = orderAddElements.length; k < len3; k++) {
      if (orderAddElements[k].product_id === product.product_id) {   
        ...
        if (mouldedSet.indexOf(addElements[orderAddElements[k].element_id].group_id) !== -1) {
          filteredAddElements.push(('&#9679 ' + translateAddElem + '<br>' + '&nbsp' + '&nbsp' + '&nbsp' + t('Width') + ' ' + orderAddElements[k].element_width + ' ' + t('mm') + ' ' + ' / ' + orderAddElements[k].element_qty + ' ' + t('pcs')));
          ...
          filteredAddElements.push(('&#9679 ' + translateAddElem + '<br>' + '&nbsp' + '&nbsp' + '&nbsp' + orderAddElements[k].element_qty + ' ' + t('pcs')));
        } else if (pieceSet.indexOf(addElements[orderAddElements[k].element_id].group_id) !== -1 && addElements[orderAddElements[k].element_id].group_id == 14) {
          filteredAddElements.push(('&#9679 ' + translateAddElem + '<br>' + '&nbsp' + '&nbsp' + '&nbsp' + t('Height') + ' ' + orderAddElements[k].element_height + ' ' + t('mm') + '. ' + (orderAddElements[k].block_id == 0? t('Left'): t('Right')) + ' / ' + orderAddElements[k].element_qty + ' ' + t('pcs')));
        } else {
          filteredAddElements.push(('&#9679 ' + translateAddElem + '<br>' + '&nbsp' + '&nbsp' + '&nbsp' + t('Width') + ' ' + orderAddElements[k].element_width + ' ' + t('mm') + ' ' + ' / ' + t('Length') + ' ' + orderAddElements[k].element_height + ' ' + t('mm') + ' ' + ' / ' + orderAddElements[k].element_qty + ' ' +t('pcs')));
        }
    ...
    let heat_coef_norma = 0;
    let heat_coef_width = 0;
    ...
    if (product.is_addelem_only !== 2 && product.is_addelem_only !== 5) {
      ...
      if (product.window_hardware_group && translateLists["doors_hardware_groups"][product.window_hardware_group.id] !== undefined) {
        ...
        filteredHardware = t('Not exist');
      }
  
      heat_coef_norma = 230 - ((+normalCoef - 0.89) / (2.84 - 0.89))* 230;
      heat_coef_width = 230 - ((product.heat_coef_total - 0.89) / (2.84 - 0.89))* 230;
    } 


    /** Form content */
    function buildRoletImgTemplate(split, sizes, sizesByType, prod, maxW = 240, maxH = 160) {
        if (!split || !sizes) {return '';}

        const w = sizes['width_total'];
        const h = sizes['height_total'];
        ...
        let mainHeightValue = h;
        let mainWidthValue = w;
        if (prod.typeSize === 2) {
          mainHeightValue = sizesByType['height_total'];
          mainWidthValue = w;
        } else if (prod.typeSize === 3) {
          mainHeightValue = sizesByType['height_total'];
          mainWidthValue = sizesByType['width_total'];
        } 

        const lamelBlocks = [];

        for (let i = 1; i <= split; i++) {
          let wblock = sizes[`width_${i}`];
          if (prod.typeSize === 3) {
            wblock = sizesByType[`width_${i}`];
          }
          const hblock = sizes[`height_${i}`];
          if (!wblock || !hblock) {continue;}

          const scaledW = Math.round(wblock * k);
          const scaledH = Math.round(hblock * k) - 20;

          const partHeightValue =
            prod.typeSize === 1
              ? sizesByType[`height_${i}`]
              : hblock;

          const countLines = Math.floor(scaledH / 10);

          const lamelLinesHtml = Array.from({ length: countLines }, () =>
            '<div class="lamel-line"></div>'
          ).join('');

          lamelBlocks.push(`
            <div class="lamel-block block-part${i}" style="width:${scaledW}px;height:${scaledH}px">
            
              <div class="left-guide">
              </div>
              <div class="center">
              ${lamelLinesHtml}
                <div class="lamel-endline">
                </div>
              </div>
              <div class="right-guide">
              </div>
              <div class="width-line">${wblock}</div>
              <div class="height-line">${partHeightValue}</div>
            </div>
          `);
        }

        const lamelsHtml = lamelBlocks.join('');

        return `
          <div class="main-rolet-img">
            <div class="rolet-img-parts" style="width:${width}px;height:${height}px">
              <div class="top-size">${mainWidthValue}</div>

              <div class="box-block">
                <div class="box-size">${prod.boxSize.height}</div>
              </div>

              <div class="wrap-lamels-block">
                ${lamelsHtml}
              </div>

              <div class="main-size-block">
                <span class="main-size">${mainHeightValue}</span>
              </div>

              <div class="main-line-size" style="height:${height - 10}px"></div>
    ...
    function buildPlsGridImg(config) {
      if (!config) {return '';}
      ...
      const width = Math.round(w * k);
      const height = Math.round(h * k);
      const img = config.color && config.color.img ? config.color.img : '';
      ...
      const directionClass = directionClasses[template.direction] || '';
      const sillClass = (template.open_type === 4 || template.open_type === 5) ? 'sill' : '';

      const frameOutClass = ['main-frame-out', directionClass].filter(Boolean).join(' ');
      const frameInClass = ['main-frame-in', sillClass].filter(Boolean).join(' ');

      const frameStyle = img
        ? `border-image-source:url('${img}');border-image-slice:30;border-image-repeat:repeat;`
        : '';
      ...
      return `
        <div class="main-pls-grid-img">
          <div class="pls-img-parts" style="width:${width}px;height:${height}px">
            <div class="size-width">${config.width}</div>
            <div class="size-height">${config.height}</div>
            <div class="${frameOutClass}">
              <div class="${frameInClass}" style="${frameStyle}"></div>
              <div class="pls-grid-sash-left">
                <div class="sash" style="${sashStyle}">
                  <div class="left-handle"></div>
                </div>
              </div>
              <div class="pls-grid-sash-right">
                <div class="sash" style="${sashStyle}">
    ...
    if (product.is_addelem_only === 2) {

      const roletHtml = buildRoletImgTemplate(
        productTemplate.split,
        productTemplate.roletSizes,
        productTemplate.sizeByType,
        productTemplate
      );
          ...
          <div class="add-container">
            <div class="add-svg-container">
              ...
              <div class="addelem-price">
                ...
                ${productTemplate.comment && productTemplate.comment.length ? 
                `<div class="rolet-comment-title">
                  ${t('Comment')}
                </div>
                <div class="rolet-comment-text">
                  ${productTemplate.comment}    
                </div>
                `
                : ''
              }
            ...
            <div class="rolet-description">
              <div class="info-titel">
                ${t('box')}
              </div>
              <span class="info-sub-titel">
                ${productTemplate.group.name},
              </span>
              <span class="info-sub-titel">
                ${productTemplate.box.name},
              </span>
              ${productTemplate.box.color && productTemplate.box.color.name ? 
                `<span class="info-sub-titel">
                  ${productTemplate.box.color.name},
                </span>` : ''
              }
              <span class="info-sub-titel">
                ${productTemplate.boxSize.width} ${t('mm')} (W) x ${productTemplate.boxSize.height} ${t('mm')} (H)
              </span>
              ${productTemplate.group.name_front && productTemplate.box.color_front && productTemplate.box.color_front.name ?
                `<br>
                <span class="info-sub-titel">
                  ${productTemplate.group.name_front + ': ' + productTemplate.box.color_front.name}
                </span>
                <br>
                <span class="info-sub-titel">
                  ${productTemplate.group.name_back + ': ' + productTemplate.box.color_back.name}
                </span>
                <br>
                <span class="info-sub-titel">
                  ${productTemplate.group.name_top + ': ' + productTemplate.box.color_top.name}
                </span>
                <br>
                <span class="info-sub-titel">
                  ${productTemplate.group.name_bottom + ': ' + productTemplate.box.color_bottom.name}
                </span>` : ''
              }
              <div class="info-titel">
                ${t('lamel')}
              ...
              <span class="info-sub-titel">
                ${productTemplate.lamel.name},
              </span>
              <span class="info-sub-titel">
                ${productTemplate.lamel.color.name}
              </span>
              <div class="info-titel">
                ${t('end_profile')} 
              </div>
              <span class="info-sub-titel">
                ${productTemplate.endList.name},
              </span>
              <span class="info-sub-titel">
                ${productTemplate.endList.color.name}
              </span>
              <div class="info-titel">
                ${t('guides')}
              </div>
              <span class="info-sub-titel">
                ${productTemplate.guide.name},
              </span>
              <span class="info-sub-titel">
                ${productTemplate.guide.color.name}
              </span>
              <div class="info-titel">
                ${t('control')}
              </div>
              <span class="info-sub-titel">
                ${productTemplate.control.name},
              </span>
              ${productTemplate.control.sides && productTemplate.control.sides.length ?
              `<span class="info-sub-titel">
                ${productTemplate.control.sides.map((s, idx) => `<span>${t('part')} ${idx+1} - ${s}</span>`).join(' ')}
              </span>` : ''
              }
              ${
                productTemplate.addElems && productTemplate.addElems.length
                  ? `
                    <div class="info-titel">
                      ${t('Add. elements')}
                    </div>
                    ${
                      productTemplate.addElems
                        .map(elem => `
                          <div class="info-sub-titel">
                            <span>${elem.name}</span>
                            <span style="margin-left: 10px">${elem.qty}</span>
                            <span>
                              ${elem.rule === 1 ? t('pcs') : ''}
                              ${elem.rule === 2 ? t('m') : ''}
                              ${elem.rule === 3 ? t('sq. m') : ''}
                              ${elem.rule === 4 ? t('unit_kg') : ''}
                              ${elem.rule === 5 ? t('mm') : ''}
                              ${elem.rule === 6 ? t('unit_cm') : ''}
                            </span>
                          </div>
                          `).join(' ')
                    }
              ...
              <div class="info-prod-price">
                <div class="ekipazh-product-count">
                  <div class="ekipazh-title">
                    ${t('Amount')} 
                  </div> 
                  <div class="ekipazh-value"> 
                    ${parseInt(product.product_qty, 10)}  ${t('pcs')} 
                  </div> 
                </div> 
                <div class="ekipazh-product-Eprice"> 
                  <div class="ekipazh-title"> 
                    ${(nds ? t('net') : t('Price')) + ' 1' + t('pcs')} 
                  </div> 
                  <div class="ekipazh-value"> 
                    ${parseFloat(productPrice/product.product_qty).toFixed(2)}  
                  </div> 
                </div> 
                <div class="ekipazh-product-Gprice"> 
                  <div class="ekipazh-title"> 
                    ${(nds ? t('net') : t('Price')) + ' ' + product.product_qty + ' ' + t('pcs')} 
                  </div> 
                  <div class="ekipazh-value"> 
    ...
    } else if (product.is_addelem_only === 5) {
      
      
      const plsGridHtml = buildPlsGridImg(
        productTemplate
      );
          ...
          <div class="add-container">
            <div class="add-svg-container">
              ...
              <div class="addelem-price">
                ...
                ${productTemplate.comment && productTemplate.comment.length ? 
                `<div class="pls-grid-comment-title">
                  ${t('Comment')}
                </div>
                <div class="pls-grid-comment-text">
                  ${productTemplate.comment}    
                </div>
                `
                : ''
            ...
            <div class="pls-grid-description" style="margin-top: 10px">
              <div class="info-titel">
                ${productTemplate.template.name}
              </div>
              <div class="info-titel">
                ${productTemplate.system.name}
              </div>
              <div class="info-titel">
                ${productTemplate.grid.name}
              </div>
              ${productTemplate.color && productTemplate.color.name ? 
                `<span class="info-titel">
                  ${productTemplate.color.name}
                </span>` : 
                `<span class="info-titel">
                  ${t('White')}
                </span>`
      ...
      '<div class="header-product">' +
        ...
        '<div class="product-description" style="margin: 0 auto">' +
          t('Size') + ': ' + parseInt(product.template_width, 10) +  t('x') + parseInt(product.template_height, 10) + ' / ' + t('Area_orders') + ': ' + parseFloat(product.template_square).toFixed(2) + ' / ' + t('Perimeter') + ' : ' + productPerimeter + ' / ' + t('Add. elements') + ': ' + addElementsQty + t('pcs') + ' / ' + (weightConstruction && weightConstruction > 0 ? (t('Weight (kg)') + ': ' + weightConstruction.toFixed(2)) : '') +
        '</div>' +
      '</div>' +
      '<div class="product">' + 
        '<div class="product-container">' +
          '<div class="product-svg-container" align="left">' +
            ...
            '<div class="info-titel">' +
              '<div>' +
                t('Thermal_conductivity') + ' U = ' + product.heat_coef_total + ' ' + t('U_coeff_short') +
              '</div>' +
            '</div>' +
            // (parseFloat(addElemPrice).toFixed(2) > 0 ?
            // '<div class="addelem-price">' +
            //   '<div class="addelem-price-name">' +
            //     t('Price of add. elements') +
            //   '</div>' +
            //   '<div class="addelem-price-value">' +
          ...
          '<div class="product-info-conteiner" >' +
            ...
            ((connectorsWidth || connectorsHeight) ?
            '<div class="info-titel">' +
              '<div>' +
                i18n.__('overall_dimensions') +
              '</div>' +
            '</div>' +
            '<div class="info-sub-titel">' +
              '<div>' +
                i18n.__('construction_extensions') +  
              '</div>' +
              '<div>' +
                i18n.__('Width') + ' ' + parseInt(product.template_width, 10) + ' + ' + connectorsWidth + ' = ' + connectorsTotalWidth + ' ' + i18n.__('mm') +
              '</div>' +
              '<div>' +
                i18n.__('Height') + ' ' + parseInt(product.template_height, 10) + ' + ' + connectorsHeight + ' = ' + connectorsTotalHeight + ' ' + i18n.__('mm') +
              '</div>' +
            '</div>' : '') +
            
            '<div class="info-titel">' +
              '<div>' +
                t('Profile system') + ' ' + product.profile_system.name +
              '</div>' +
            '</div>' +
            (product.construction_type === 4 ?
            '<div class="info-sub-titel">' +
              '<div>' +
                (product.door_type_index === 0 ? t('Frame type') + ': ' + t('Perimeter frame') : '') +
                (product.door_type_index === 1 ? t('Frame type') + ': ' + t('Frame without threshold') : '') +
                (product.door_type_index === 3 || product.door_type_index === 2 ? t('Frame type') + ': ' + t('Frame with aluminum threshold') : '') +
              '</div>' +
            '</div>' : '' ) +
            '<div class="info-sub-titel">' +
              '<div>' +
                t('Amount of cameras') + ': ' + qtyCameras + ' / ' +
                t('Thermal_conductivity') + ': ' + heatCoeffProfile.toFixed(2) + 
                // t('Country') + ': ' + countryProfile +
              '</div>' +
            '</div>' +
            '<div class="info-sub-titel">' +
              '<div>' +
                t('Profile color') + ' ' + (filteredLaminations.length ? filteredLaminations.join(', ') : t('Not exist')) +
              '</div>' +
            '</div>' +
            ...
            filteredGlasses.map((glass, index) => (
              '<div class="info-titel">' +
                '<div>' +
                  t('Glazed window') + ' ' + glass +
                '</div>' +
              '</div>' +
              '<div class="info-sub-titel">' +
                '<div>' +
                  t('Width') + ' ' + prodGlassInfo[index].gl_width + t('mm') + ' / ' + t('Noise isolation (dB)') + ': ' + prodGlassInfo[index].gl_noise_coeff + ' / ' + '<br>' + t('Thermal_conductivity') + ': ' + (1/prodGlassInfo[index].gl_transcalency).toFixed(2) + 
                '</div>' +
              '</div>' + 
              '<div class="info-sub-titel">' +
                '<div>' +
                  t('Glazed window type') + ': ' + prodGlassInfo[index].gl_list_folders.map(fl=>fl[currentLanguage]).join(' / ') + 
                '</div>' +
              '</div>'
            )).join('') +

            (productTemplate.shprosData && productTemplate.shprosData.type ? 
              '<div class="info-sub-titel">' +
                '<div>' +
                  i18n.__('sh_muntins') + ': ' + productTemplate.shprosData.width + ' ' + i18n.__('mm') + ' '  + (productTemplate.shprosData.type === 1 ? 
                    i18n.__('sh_internal') : 
                    productTemplate.shprosData.type === 2 ? 
                    i18n.__('sh_external') : 
                    i18n.__('sh_struct')) + ' ' + productTemplate.shprosData.curLength + ' ' + i18n.__('m') +
                '</div>' +
              '</div>' : ''
            ) +
            ...
            '<div class="info-sub-titel">' +
              '<div>' +
                t('Height to handle') +
                ' : ' + heightHandle + (heightHandle === 'Center' ? "" : t('mm')) + ' / ' +
                (decorName !==undefined ? 
                  (t('Color of decors') +
                  ' : ' + decorName ) : ' '
                ) + 
            ...
            '<div class="info-sub-titel">' +
              '<div>' +
                doorHingeCount + t('pcs') + ' ' + doorHingeName +
              '</div>' +
            '</div>' : ' ') +
            // test price right
            '<div class="info-prod-price">' +
            ...
            '<div class="info-sub-titel">' +
              '<div>' +
                (filteredAddElements.length ? filteredAddElements.join('<br>') : t('Not exist')) +
              '</div>' +
            '</div>' +
          '</div>' +
        '</div>' +
  ...
  function __formAdditionalContent (additionalContent, structCounter, addPriceDisc, locale) {
    var loc = locale || i18n.getLocale();
    var t = function (phrase) { return i18n['__']({ phrase: phrase, locale: loc }); };
    var divider = '';
