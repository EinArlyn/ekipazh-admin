...
function formContent (order, factory, userDiscounts, currUserId, additionalProductsIds, callback) {
   ...
   */ 
  function __formProductContent (orderAddElements, product, structCounter, laminations, addElements, glasses, productPrice, addElemPrice, translateLists, prodGlassInfo, dataObj, normalCoef) {
    var currentLanguage = i18n.getLocale();
    ...
    /** Form content */
    return content = divider + `<div class="bound" style="max-height: ${addElemProdQty > 4 ? 740 : 370}px; min-height: ${addElemProdQty > 4 ? 740 : 370}px;">` +
      ...
      '<div class="product-container">' +
        ...
        '<div class="product-info-conteiner" >' +
          ...
          filteredGlasses.map((glass, index) => (
            ...
            '<div class="info-sub-titel">' +
              '<div>' +
                i18n.__('Width') + ' ' + prodGlassInfo[index].gl_width + i18n.__('mm') + ' / ' + i18n.__('Noise isolation (dB)') + ': ' + prodGlassInfo[index].gl_noise_coeff + ' / ' + i18n.__('Thermal_conductivity') + ': ' + (1/prodGlassInfo[index].gl_transcalency).toFixed(2) + 
              '</div>' +
            '</div>' + 
            '<div class="info-sub-titel">' +
              '<div>' +
          ...
          '<div class="info-titel">' +
            '<div>' +
              i18n.__('Thermal_conductivity') + ' ' + product.heat_coef_total +
            '</div>' +
          '</div>' +
