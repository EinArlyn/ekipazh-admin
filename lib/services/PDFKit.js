...
function formContent (order, factory, userDiscounts, additionalProductsIds, callback) {
  ...
  parseOrder(order.order, factory, function (parsedOrder) {
    models.locales_names.findAll({
      where: { factory_id: factory, table_name: 'lists' }
    }).then(function (locales_names) {
      const filteredDataValues = locales_names.filter(item => item.dataValues.table_name === "lists" && item.dataValues.table_attr === "name").reduce((acc, item) => {
      ...
      async.eachSeries(products, function (product, cb) {
        // console.log('ORDER>>>>>>>>', order.user.dataValues.currencies_id)
  ...
  function __formProductContent (orderAddElements, product, structCounter, laminations, addElements, glasses, productPrice, addElemPrice, translateLists) {
    ...
    var content = '';
    var divider = '';
    var filteredHardware = (product.window_hardware_group ? product.window_hardware_group.name : i18n.__('Not exist'));
    ...
    for (var j = 0, len2 = laminations.length; j < len2; j++) {
      ...
      if (product.lamination_in_id === laminations[j].lamination_type_id) {
        filteredLaminations.push((laminations[j].name + ' (' + i18n.__('inner') + ')'));
      }

      if (product.lamination_out_id === laminations[j].lamination_type_id) {
        filteredLaminations.push((laminations[j].name + ' (' + i18n.__('outer') + ')'));
      }
    }
    ...
    for (var k = 0, len3 = orderAddElements.length; k < len3; k++) {
      if (orderAddElements[k].product_id === product.product_id) {    
        ...
        if (mouldedSet.indexOf(addElements[orderAddElements[k].element_id].group_id) !== -1) {
          filteredAddElements.push((translateLists[orderAddElements[k].element_id][currentLanguage] + ' (' + orderAddElements[k].element_width + ',' + orderAddElements[k].element_qty + ' ' + i18n.__('pcs') +')'));
        } else if (pieceSet.indexOf(addElements[orderAddElements[k].element_id].group_id) !== -1) {
          filteredAddElements.push((translateLists[orderAddElements[k].element_id][currentLanguage] + ' (' + orderAddElements[k].element_qty + ' ' + i18n.__('pcs') +')'));
        } else {
          filteredAddElements.push((translateLists[orderAddElements[k].element_id][currentLanguage] + ' (' + orderAddElements[k].element_width + 'х' + orderAddElements[k].element_height + ',' + orderAddElements[k].element_qty + ' ' +i18n.__('pcs') +')'));
        }
      }
    }
    ...
    product.profile_system = product.profile_system || product.doors_group;
