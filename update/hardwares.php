...
foreach ($hardwares['groups'] as $group_id => $group) 
{
	...
	foreach ($hardwares['groups'][$group_id]['hardware_sys'] as $hardware_id => $hardware) 
	{
		...
		if ($hardware['is_in_calculation']==0)
		{
			...
			echo " <input onchange=\"ajax_post('hardware_name=$hardware_id&value='+this.value,'');\" style='width:62%;float:left;font-size: 15px;background:#ff643a;' type=text value='$hardware_name'>";
		}	
		else
		{			
