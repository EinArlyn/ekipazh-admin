...
router.get('/', isAuthenticated, rolet.lamelPriceColor);
...
function getPrices(req, res) {
    models.rol_lamel_prices.findAll({}).then(function(prices){
...
function changePrice(req, res) {
  ...
  if (!Number.isFinite(groupColorId) || !Number.isFinite(lamelId) || !Number.isFinite(newPrice) || !Number.isFinite(rules)) {
    return res.send({ status: false, error: 'bad params' });
  }

  models.rol_lamel_prices.findOne({
    where: { rol_lamel_id: lamelId, rol_color_group_id: groupColorId }
  })
  .then(row => {
    if (row) {
    ...
    return models.rol_lamel_prices.create({
...
function globalChangeRules(req, res) {
  ...
  Promise.all(values.map(v =>
    models.rol_lamel_prices.update(
      { rol_price_rules_id: rules },
      { where: { rol_lamel_id: v.lamelId, rol_color_group_id: v.groupColorId } }
    )
  ))
