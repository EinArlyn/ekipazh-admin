...
router.get('/', isAuthenticated, rolet.roletGuideColors);
...
function updateRow(req, res) {
    ...
    models.rol_guide_color_groups.findOne({
        where: {rol_guide_id: guideId, rol_color_id: colorId}
    }).then(function(row) {
        if (row) {
            ...
            return models.rol_guide_color_groups.create({
